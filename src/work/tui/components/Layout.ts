...
export class Layout {
  ...
  private trashKeyHandler: ((ch: string, key: any) => void) | null = null;
  ...
  private focusConversation(): void {
    ...
    // Cancel first to prevent double input handlers on Windows
  ...
  private linkPickerKeyHandler: ((ch: string, key: any) => void) | null = null;
  ...
  private closeOverlay(): void {
    ...
    if (this.linkPickerOverlay) {
      // Remove the key handler when closing
      if (this.linkPickerKeyHandler) {
        this.screen.removeListener('keypress', this.linkPickerKeyHandler);
        this.linkPickerKeyHandler = null;
      }
  ...
  showLinkPicker(urls: string[], onOpen: (url: string) => void, onCopy: (url: string) => void): void {
    if (this.linkPickerOverlay) {
      // Remove the old keyHandler before destroying overlay
      if (this.linkPickerKeyHandler) {
        this.screen.removeListener('keypress', this.linkPickerKeyHandler);
        this.linkPickerKeyHandler = null;
      }
    ...
    // Single keypress handler for all keys - store as instance variable
    this.linkPickerKeyHandler = (ch: string, key: any) => {
    ...
    this.screen.on('keypress', this.linkPickerKeyHandler);
  ...
  private memoryKeyHandler: ((ch: string, key: any) => void) | null = null;
  ...
  showMemoryManager(
    ...
    if (this.memoryOverlay) {
      // Remove the old keyHandler before destroying overlay
      if (this.memoryKeyHandler) {
        this.screen.removeListener('keypress', this.memoryKeyHandler);
        this.memoryKeyHandler = null;
      }
    ...
    // Cancel textarea input and hide it to prevent doubled characters
    (this.inputBox as any).cancel?.();
    this.inputBox.hide();
  ...
  ${actions}  {gray-fg}[Backspace]{/gray-fg} Back  {gray-fg}[Esc]{/gray-fg} Close
    ...
    // Key handler - store as instance variable so it can be removed in toggle case
    this.memoryKeyHandler = async (ch: string, key: any) => {
      if (key.name === 'escape') {
        cleanup();
        return;
      }
    ...
    this.screen.on('keypress', this.memoryKeyHandler);
    contentBox.focus();
    renderMenu();
  }
  ...
  async showTrashBin(): Promise<void> {
    if (this.trashOverlay) {
      // Remove the old keyHandler before destroying overlay
      if (this.trashKeyHandler) {
        this.screen.removeListener('keypress', this.trashKeyHandler);
        this.trashKeyHandler = null;
      }
    ...
    // Key handler - store as instance variable so it can be removed in toggle case
    this.trashKeyHandler = async (ch: string, key: any) => {
    ...
    this.screen.on('keypress', this.trashKeyHandler);
    contentBox.focus();
    renderList();
  }

  private characterOverlay: blessed.Widgets.BoxElement | null = null;
  private characterKeyHandler: ((ch: string, key: any) => void) | null = null;
  ...
  async showCharacterSelector(
    ...
    if (this.characterOverlay) {
      // Remove the old keyHandler before destroying overlay
      if (this.characterKeyHandler) {
        this.screen.removeListener('keypress', this.characterKeyHandler);
        this.characterKeyHandler = null;
      }
    ...
    const cleanup = () => {
      ...
      // Show and re-focus input - don't call cancel() as it can error if no input is active
      this.inputBox.show();
      this.inputBox.focus();
      this.screen.render();
    };
    ...
    // Store as instance variable so it can be removed in toggle case
    this.characterKeyHandler = async (ch: string, key: any) => {
      ...
      if (key.name === 'escape' || ch === 'q') {
        ...
        if (selected.type === 'new_custom') {
          // Show custom input prompt - cleanup before showing new dialog
          cleanup();
          this.showCustomCharacterPrompt(onSelect);
        } else if (selected.type === 'custom' && selected.customDesc) {
          // Use existing custom character
          // IMPORTANT: await onSelect BEFORE cleanup to avoid "done is not a function" error
          // Cleanup restores focus to inputBox, which can error if done while async is pending
          await onSelect('custom', selected.customDesc);
          cleanup();
        } else {
          // Same pattern: await first, then cleanup
    ...
    // Use screen.on for keypress to intercept all keys
    this.screen.on('keypress', this.characterKeyHandler);
