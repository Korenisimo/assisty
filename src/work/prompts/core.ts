// Core system prompt - MINIMAL (~100 tokens target)
// All detailed guidance moved to modules loaded conditionally:
// - Communication → communicationModule
// - Tool selection → toolSelectionModule  
// - Workflows → workflowPatternsModule
...
export async function getCorePrompt(personality: PersonalityConfig, character: CharacterConfig): Promise<string> {
  ...
  const personalityInstructions = personality.type === 'proactive' 
    ...
    ? `\nBe minimal - brief responses, mention tasks only when asked.`
    : '';
  ...
  // MINIMAL CORE - only the 5 most critical rules
